...
from brain.application.interactors.auth.dto import DecodedJwtTokenPayload, JwtTokenCreationPayload
...
from brain.domain.entities.jwt import FullJwtToken, JwtAccessToken, JwtRefreshToken
...
class AuthTokensService:
    ...
    def _create_access_token_for_user(self, user_id: UUID) -> JwtAccessToken:
        return self._create_jwt_token(payload=JwtTokenCreationPayload(user_id=user_id))

    async def _create_refresh_token_for_user(self, user_id: UUID) -> JwtRefreshToken:
        refresh_expires_at = utc_now() + timedelta(seconds=self._auth_config.refresh_token_lifetime)
        refresh_jwt = self._jwt_service.create_token(payload={"user_id": user_id, "exp": refresh_expires_at})
    ...
    def _decode_jwt_token(self, token: str) -> DecodedJwtTokenPayload:
        ...
        return DecodedJwtTokenPayload(**data)
    ...
    async def issue_refresh_token_for_telegram_id(self, telegram_id: int) -> JwtRefreshToken:
        user = await self._user_lookup_service.get_user_by_telegram_id(telegram_id)
    ...
    async def authorize_by_token(self, token: str) -> User:
        ...
        return await self._user_lookup_service.get_user_by_id(payload.user_id)
